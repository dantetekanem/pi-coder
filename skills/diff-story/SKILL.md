...
Arrange code for a human to review. The host has extracted changed functions, test cases and supporting hunks from frozen files. It supplies stable unit IDs, filenames, symbol names, local reference matches and likely test pairs.

Choose a useful reading order and correct any clear pairing mistakes. Make one pass over this prepared map. Each test belongs to one implementation step or one independent step. Use source order where several reading orders work equally well.
