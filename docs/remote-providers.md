...
In an opened thread, `v` reveals its code location only when its fetched head matches the immutable review header and file comparison, its side/path match exactly, and the loaded side has that line. Deleted-side navigation also requires a matching immutable base. Missing, outdated, or unverified anchors leave code position unchanged. Unloaded files are checked before navigation; later input, refresh, disposal, or changed comparison cancels a pending jump. Valid unchanged lines are revealed outside the initial hunks. GraphQL thread fields carry their fetched PR head/base; REST uses `commentSide` (`LEFT`/`RIGHT` or `deleted`/`added`) and `commentCommitId` (GitHub `side`/`commit_id`), without inventing a base. This checks revision/path metadata and loaded line availability, not a cryptographic byte comparison or an atomic provider snapshot.
